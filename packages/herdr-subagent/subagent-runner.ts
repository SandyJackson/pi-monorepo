...
import { formatMergedAgentList, resolveAgent } from "./agents.ts";
// ---------------------------------------------------------------------------
  ...
  switch (value.status) {
    ...
    case "failed":
      return typeof value.error === "string"
        ...
        : null;
    case "not_started":
      return (value.reason === "aborted" || value.reason === "catastrophic") &&
...
function attemptFailureResult(
  task: ResolvedTask,
  attempt: Exclude<SpawnAttempt, { status: "spawned" }>,
): ParallelResult {
...
function formatParallelResults(results: ParallelResult[]): string {
...
export async function runSubagents(
  ...
  const tasks = Array.isArray(tasksParam) ? (tasksParam as Array<Record<string, unknown>>) : null;
  ...
  const timeoutMinutes =
    typeof params.timeout === "number" && Number.isInteger(params.timeout) && params.timeout > 0
      ? params.timeout
...
async function runParallel(
  ...
  const recordResult = (task: ResolvedTask, result: ParallelResult) => {
    ...
    onUpdate?.({
      content: [
        { type: "text", text: `parallel — ${settledCount}/${taskInfos.length} tasks settled…` },
      ],
      details: {},
    });
  };
  ...
  for (let batchStart = 0; batchStart < taskInfos.length; batchStart += MAX_CONCURRENCY) {
    if (signal?.aborted) {
      markRemainingNotStarted(
        batchStart,
        "aborted",
        "delegation was aborted before this task launched",
      );
      break;
    }
    ...
    for (const task of batchTasks) {
      ...
      if (!attempt) {
        recordResult(
          task,
          notStartedResult(task, "catastrophic", "backend omitted this task result"),
        );
        continue;
      }
    ...
    await Promise.all(
      spawnedAttempts.map(async (attempt) => {
