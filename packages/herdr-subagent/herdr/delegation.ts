...
async function findOrCreateSubagentsTab(
	...
	const listResult = (await rpc("tab.list", { workspace_id: workspaceId }, DEFAULT_RPC_TIMEOUT, signal)) as { tabs?: HerdrTabInfo[] };
...
async function findAnyTabPane(
	...
	const result = (await rpc("pane.list", { workspace_id: workspaceId }, DEFAULT_RPC_TIMEOUT, signal)) as { panes?: HerdrPaneInfo[] };
		...
		for (let index = 0; index < tasks.length; index++) {
			...
			if (!targetPaneId || targetPaneCwd !== task.cwd) {
				if (!splitAnchorPaneId) {
					outcomes[index] = { status: "launch_failed", error: "subagents tab has no pane to split" };
					continue;
				}
				try {
