...
describe("herdr/session — launch invariants", () => {
  ...
  it("caps label at 32 and retries name collisions up to 100", async () => {
    ...
    const herdr = createScriptedHerdr((req) => {
      ...
      if (req.method === "agent.list")
        return { result: { agents: [{ agent: "sa-alpha-hello-world" }] } };
      if (req.method === "agent.start") {
        startCalls += 1;
  ...
  it("classifies confirmed launch failure vs indeterminate (no pane_id, transport loss)", async () => {
    const confirmed = createScriptedHerdr(
      (req) =>
    ...
    const indeterminate = createScriptedHerdr(
      (req) => standardLaunchResponse(req) ?? { result: {} },
    );
    ...
    const transport = createScriptedHerdr(
      (req) => standardLaunchResponse(req) ?? { closeWithoutResponse: true },
    );
  ...
  it("reports a confirmed session with observeTurn on successful launch", async () => {
    ...
    controller.abort();
    await expect(launch.observeTurn({ signal: controller.signal })).resolves.toMatchObject({
      status: "aborted",
      stage: "observing",
    });
  });
  ...
  it("aborts before launch when signal already aborted", async () => {
    const herdr = createScriptedHerdr(
      (req) => standardLaunchResponse(req) ?? { result: { pane_id: "p" } },
    );
  ...
  it("keeps the prompt until observation ends, then expires it via the fallback", async () => {
    ...
    const session = launchAndObserve({
      ...
      task: {
        ...
        instruction: "t",
        cwd: "/tmp",
        config: { name: "alpha", systemPromptBody: "body-slow" },
      },
      signal: controller.signal,
    });
  ...
  it("indeterminate retains 60s and confirmed retains until session path", async () => {
    ...
    const p1 = launchAndObserve({
      ...
      task: {
        ...
        instruction: "t",
        cwd: "/tmp",
        config: { name: "alpha", systemPromptBody: "body-ind" },
      },
    });
    await p1;
    const dir = [...promptDirs()].find((e) => !before.has(e));
    ...
    const sessionPath = writeSessionFile(
      "prompt-session",
      jsonl(
        { type: "session", id: "s1", cwd: "/tmp", timestamp: "x" },
        {
          type: "message",
          id: "m1",
          parentId: null,
    ...
    const confirmed = createScriptedHerdr((req) => {
      ...
      if (getCount === 1)
        return {
          result: { agent: { agent_status: "working", agent_session: { path: sessionPath } } },
        };
    ...
    const session = launchAndObserve(
      {
        ...
        task: {
          ...
          instruction: "t2",
          cwd: "/tmp",
          config: { name: "alpha", systemPromptBody: "body-conf" },
        },
      },
      { timeoutMs: 5000 },
    );
...
describe("herdr/session — observation", () => {
  ...
  it("partial header: session path published but header not yet readable retries", async () => {
    ...
    const fullPath = writeSessionFile(
      "partial-full",
      jsonl(
        { type: "session", id: "s-partial", cwd: "/tmp", timestamp: "x" },
        {
          type: "message",
          id: "m-partial",
          parentId: null,
    ...
    const herdr = createScriptedHerdr((req) => {
      ...
      if (calls === 1)
        return {
          result: { agent: { agent_status: "working", agent_session: { path: emptyPath } } },
        };
      if (calls === 2)
        return {
          result: { agent: { agent_status: "working", agent_session: { path: fullPath } } },
        };
    ...
    const run = launchAndObserve(
      {
        ...
        task: {
          ...
          instruction: "t",
          cwd: "/tmp",
          config: { name: "alpha", systemPromptBody: "body-clean" },
        },
      },
      { timeoutMs: 800 },
    );
  ...
  it("exhausts 100 name attempts", async () => {
    const herdr = createScriptedHerdr((req) => {
      ...
      if (std) return std;
      if (req.method === "agent.start")
        return { error: { code: "agent_name_taken", message: "taken" } };
      return { result: {} };
    });
  ...
  it("exhausts 10 pane-busy attempts", async () => {
    const herdr = createScriptedHerdr((req) => {
      ...
      if (std) return std;
      if (req.method === "agent.start")
        return { error: { code: "agent_pane_busy", message: "busy" } };
      return { result: {} };
    });
    vi.useFakeTimers({ toFake: ["setTimeout", "clearTimeout", "Date"] });
    const run = launchAndObserve({
  ...
  it("in-flight cancellation after agent.start becomes launch_indeterminate", async () => {
    const herdr = createScriptedHerdr((req) => {
      ...
      if (std) return std;
      if (req.method === "agent.start")
        return { leavePending: true } as import("../herdr-test-support.ts").RpcResponse;
      return { result: {} };
    });
    const ctrl = new AbortController();
    const run = launchAndObserve({
