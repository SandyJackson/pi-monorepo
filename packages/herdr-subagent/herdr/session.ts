...
function buildAgentArguments(task: DelegatedTask, promptArgs: string[]): string[] {
  const argv: string[] = [];
...
async function allocateAndStartAgent(
  ...
  const occupied = new Set(
    listed.agents.map((e) => e.agent?.trim()).filter((n): n is string => Boolean(n)),
  );

  const argv = buildAgentArguments(task, promptArgs);

  for (
    let ordinal = 1, attempts = 0;
    attempts < MAX_NAME_ALLOCATION_ATTEMPTS;
    attempts++, ordinal++
  ) {
...
function isSessionClosedError(error: unknown): boolean {
  if (error instanceof HerdrRpcResponseError) {
    return (
      error.code === "not_found" || error.code === "pane_not_found" || error.code === "no_such_pane"
    );
  }
  return false;
}
...
function readAgentObservation(
  info: unknown,
): { status: string; sessionPath: string | null } | { invalid: string } {
  ...
  if (
    ...
    (typeof agentPayload !== "object" || agentPayload === null || Array.isArray(agentPayload))
  ) {
...
async function observeTurnUntilSettled(
  options: ObserveTurnUntilSettledOptions,
  ...
  try {
    for (;;) {
      if (signal?.aborted)
        return { status: "aborted", stage: "observing", session: observedSession };
      ...
      try {
        ...
        if ("invalid" in observed)
          return {
            ...
            error: observed.invalid,
          };
        status = observed.status;
        sessionPath = observed.sessionPath;
      } catch (err) {
        if (isSessionClosedError(err))
          return { status: "session_closed", session: observedSession };
        if (err instanceof HerdrRpcResponseError)
          return { status: "observation_failed", session: observedSession, error: err.message };
      ...
      onProgress?.(
        `watching:${status ?? "unknown"} — pane ${paneId}${answer ? " (final answer captured)" : ""}`,
      );
 ...
 */
export async function launchDelegatedTask(
  options: LaunchDelegatedTaskOptions,
): Promise<DelegatedTaskLaunch> {
