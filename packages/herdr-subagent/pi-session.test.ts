...
describe("pi-session Pi identity", () => {
	it("first available path plus valid header yields Pi session id, absolute path, and cwd atomically", () => {
		const sessionPath = writeSessionFile(
			"valid.jsonl",
			jsonl(
				{ type: "session", id: "sess-xyz", cwd: "/tmp/project", timestamp: "2024-01-01T00:00:00.000Z" },
				{
					type: "message",
					id: "message-1",
					parentId: null,
	...
	it("malformed header — first entry not session — yields no pi", () => {
		const sessionPath = writeSessionFile(
			...
			jsonl({ type: "message", id: "message-1", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "hi" }] } }),
		);
		const snapshot = inspectSession(sessionPath);
	...
	it("header missing id yields no pi", () => {
		const sessionPath = writeSessionFile(
			...
			jsonl(
				...
				{ type: "message", id: "message-1", parentId: null, timestamp: "x", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "hi" }] } },
			),
		);
	...
	it("header with empty or whitespace id/cwd yields no pi", () => {
		const emptyId = writeSessionFile("empty_id.jsonl", jsonl({ type: "session", id: "", cwd: "/c", timestamp: "x" }));
		expect(inspectSession(emptyId).pi).toBeNull();
		const wsId = writeSessionFile("ws_id.jsonl", jsonl({ type: "session", id: "   ", cwd: "/c", timestamp: "x" }));
		expect(inspectSession(wsId).pi).toBeNull();
		const emptyCwd = writeSessionFile("empty_cwd.jsonl", jsonl({ type: "session", id: "s1", cwd: "", timestamp: "x" }));
		expect(inspectSession(emptyCwd).pi).toBeNull();
		const wsCwd = writeSessionFile("ws_cwd.jsonl", jsonl({ type: "session", id: "s1", cwd: "   ", timestamp: "x" }));
	...
	it("blank lines before valid header are skipped, but malformed first header yields no pi", () => {
		const withBlanks = writeSessionFile("skip_blanks.jsonl", '   \n\n' + jsonl({ type: "session", id: "session-2", cwd: "/cwd2", timestamp: "x" }));
		expect(inspectSession(withBlanks).pi?.id).toBe("session-2");

		const malformedFirst = writeSessionFile("skip_malformed.jsonl", 'not json\n' + jsonl({ type: "session", id: "session-2", cwd: "/cwd2", timestamp: "x" }));
	...
	it("null, array, and primitive JSON lines do not throw and are skipped", () => {
		const sessionPath = writeSessionFile(
			...
			'null\n42\n\"string\"\n[]\n' +
				...
				'null\n' +
				jsonl({ type: "message", id: "message-1", parentId: null, timestamp: "x", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "hi" }] } }),
		);
		...
		const cleanPath = writeSessionFile(
			...
			jsonl({ type: "session", id: "session-1", cwd: "/c", timestamp: "x" }) + 'null\n42\n' + jsonl({ type: "message", id: "message-1", parentId: null, timestamp: "x", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "hi" }] } }),
		);
	...
	it("oversized line (>1MiB) is skipped and does not hide following valid entries", () => {
		...
		const sessionPath = writeSessionFile("oversized.jsonl", oversized + "\n" + jsonl({ type: "session", id: "session-1", cwd: "/c", timestamp: "x" }, { type: "message", id: "message-1", parentId: null, timestamp: "x", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "hi" }] } }));
		...
		const validAfterOversized = writeSessionFile("oversized_then_valid.jsonl", jsonl({ type: "session", id: "session-1", cwd: "/c", timestamp: "x" }) + oversized + "\n" + jsonl({ type: "message", id: "message-1", parentId: null, timestamp: "x", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "hi" }] } }));
		...
		const multiPath = writeSessionFile("multi_oversized.jsonl", multiOversized + "\n" + jsonl({ type: "session", id: "session-1", cwd: "/c", timestamp: "x" }));
	...
	it("oversized physical line with valid JSON suffix on same line is fully discarded", () => {
		...
		const validSuffix = JSON.stringify({ type: "message", id: "message-1", parentId: null, timestamp: "x", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "injected" }] } });
		const oversizedWithSuffix = padding + validSuffix; // same physical line, no newline between padding and JSON
		const sessionPath = writeSessionFile("oversized_suffix.jsonl", jsonl({ type: "session", id: "session-1", cwd: "/c", timestamp: "x" }) + oversizedWithSuffix + "\n" + jsonl({ type: "message", id: "message-2", parentId: null, timestamp: "x", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "real" }] } }));
	...
	it("oversized final line without trailing newline is discarded", () => {
		...
		const sessionPath = writeSessionFile("oversized_eof.jsonl", jsonl({ type: "session", id: "session-1", cwd: "/c", timestamp: "x" }) + oversizedNoNewline);
		...
		const withValidBefore = writeSessionFile("valid_before_oversized_eof.jsonl", jsonl({ type: "session", id: "session-1", cwd: "/c", timestamp: "x" }, { type: "message", id: "message-1", parentId: null, timestamp: "x", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "hi" }] } }) + oversizedNoNewline);
...
describe("pi-session answer reference capture", () => {
	it("stable settlement can capture exact {path, entryId} or explicit absence", () => {
		const sessionPath = writeSessionFile(
			...
			jsonl(
				...
				{ type: "message", id: "message-1", parentId: null, timestamp: "x", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "first" }] } },
			),
		);
		const snapshot = inspectSession(sessionPath);
		expect(snapshot.answer).toEqual({ path: sessionPath, entryId: "message-1" });

		const emptyPath = writeSessionFile("answerless.jsonl", jsonl({ type: "session", id: "session-1", cwd: "/c", timestamp: "x" }));
	...
	it("preserves substantive/whitespace selection: latest substantive wins, whitespace only if no substantive", () => {
		const sessionPath = writeSessionFile(
			...
			jsonl(
				{ type: "session", id: "session-1", cwd: "/c", timestamp: "x" },
				{ type: "message", id: "message-1", parentId: null, timestamp: "x", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "   " }] } },
				{ type: "message", id: "message-2", parentId: "message-1", timestamp: "x", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "real" }] } },
				{ type: "message", id: "message-3", parentId: "message-2", timestamp: "x", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "   " }] } },
			),
		);
		expect(inspectSession(sessionPath).answer?.entryId).toBe("message-2");
	...
	it("whitespace-only answer is explicit absence when no text", () => {
		const sessionPath = writeSessionFile(
			...
			jsonl(
				{ type: "session", id: "session-1", cwd: "/c", timestamp: "x" },
				{ type: "message", id: "message-1", parentId: null, timestamp: "x", message: { role: "assistant", stopReason: "stop", content: [] } },
			),
		);
		expect(inspectSession(sessionPath).answer).toBeNull();
	});
	...
	it("no terminal answer yields null", () => {
		const sessionPath = writeSessionFile(
			...
			jsonl(
				{ type: "session", id: "session-1", cwd: "/c", timestamp: "x" },
				{ type: "message", id: "message-1", parentId: null, timestamp: "x", message: { role: "assistant", stopReason: "toolUse", content: [{ type: "text", text: "hi" }] } },
				{ type: "message", id: "message-2", parentId: "message-1", timestamp: "x", message: { role: "user", content: "hi" } },
			),
		);
		expect(inspectSession(sessionPath).answer).toBeNull();
	});
	...
	it("end_turn is treated as terminal like stop", () => {
		const sessionPath = writeSessionFile(
			...
			jsonl(
				{ type: "session", id: "session-1", cwd: "/c", timestamp: "x" },
				{ type: "message", id: "message-1", parentId: null, timestamp: "x", message: { role: "assistant", stopReason: "end_turn", content: [{ type: "text", text: "done" }] } },
			),
		);
		expect(inspectSession(sessionPath).answer?.entryId).toBe("message-1");
	...
	it("requires nonempty persisted id", () => {
		const sessionPath = writeSessionFile(
			...
			jsonl(
				{ type: "session", id: "session-1", cwd: "/c", timestamp: "x" },
				{ type: "message", parentId: null, timestamp: "x", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "hi" }] } },
				{ type: "message", id: "", parentId: null, timestamp: "x", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "hi2" }] } },
				{ type: "message", id: "   ", parentId: null, timestamp: "x", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "hi3" }] } },
			),
		);
		expect(inspectSession(sessionPath).answer).toBeNull();
	});
		...
		const sessionPath = writeSessionFile(
			...
			jsonl(
				{ type: "session", id: "session-1", cwd: "/c", timestamp: "x" },
				{
					...
					parentId: null,
					timestamp: "x",
					message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "hello " }, { type: "text", text: "world" }, { type: "thinking", text: "ignored" }] },
				},
			),
		);
	...
	it("handles string content", () => {
		const sessionPath = writeSessionFile(
			...
			jsonl(
				{ type: "session", id: "session-1", cwd: "/c", timestamp: "x" },
				{ type: "message", id: "message-1", parentId: null, timestamp: "x", message: { role: "assistant", stopReason: "stop", content: "string answer" } },
			),
		);
...
describe("pi-session resolution", () => {
	it("resolution finds only that persisted entry after later appends/branches", () => {
		...
		fs.appendFileSync(
			sessionPath,
			jsonl(
				{ type: "message", id: "message-2", parentId: "message-1", timestamp: "x", message: { role: "user", content: "next" } },
				{ type: "message", id: "message-3", parentId: "message-2", timestamp: "x", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "second" }] } },
			),
		);
	...
	it("malformed referenced entry yields null", () => {
		...
		const wrongRolePath = writeSessionFile(
			...
			jsonl(
				{ type: "session", id: "session-1", cwd: "/c", timestamp: "x" },
				{ type: "message", id: "message-1", parentId: null, timestamp: "x", message: { role: "user", stopReason: "stop", content: [{ type: "text", text: "hi" }] } },
			),
		);
