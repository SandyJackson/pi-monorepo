...
export class HerdrBackend implements SubagentBackend {
	...
	async spawnBatch(
		...
		const batchResult = await this.withSubagentsTabLock(workspaceId, options.signal, async () => {
			...
			try {
				subagentsTab = await this.findOrCreateSubagentsTab(workspaceId, invocations[0].cwd, options.signal);
			} catch (err) {
				if (options.signal?.aborted) return { attempts: abortedAttempts(invocations) };
				throw err;
			}
			...
			for (let i = 0; i < invocations.length; i++) {
				...
				try {
					...
					if (err instanceof IndeterminateLaunchFailure) {
						...
						targetPaneId = undefined;
						hasPlacedChild = true;
						attempts.push({ invocationId: invocation.invocationId, status: "indeterminate", error });
					} else {
		...
		for (;;) {
			...
			try {
				...
				if (
					...
					return { reason: "target_closed", fallbackText: `(${spawned.displayTarget} closed before completion)` };
				}
	...
	private async findOrCreateSubagentsTab(
		...
		if (!tabId) {
			throw new Error(`tab.create returned no tab_id:\n${JSON.stringify(createResult).slice(0, 300)}`);
		}

		const rootPaneId = createResult.root_pane?.pane_id;
		if (!rootPaneId) {
			throw new Error(`tab.create returned no root_pane.pane_id:\n${JSON.stringify(createResult).slice(0, 300)}`);
		}
	...
	private async splitPane(
		...
		if (!paneId) throw new Error(`pane.split returned no new pane id:\n${JSON.stringify(result).slice(0, 300)}`);
		return paneId;
	}
...
function buildPaneLabel(agentName: string, task: string, ordinal = 1): string {
	...
	const readable =
		((readableSlug || "agent").slice(0, Math.max(1, availableReadableLength)).replace(/-+$/, "") || "agent");
