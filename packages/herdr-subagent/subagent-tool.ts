...
import type {
	DelegatedTaskExecution,
	DelegatedTaskRecord,
} from "./herdr/delegation.ts";
...
export function createSubagentTool(
	...
	const agentsSuffix = options.agents.length > 0
	...
	return {
		...
		async execute(
			...
			for (const task of parsedTasks) {
				if (!task.agent) {
					resultsByTaskNumber.set(task.taskNumber, {
						task,
						outcome: { status: "invalid", reason: "empty-agent", error: "Agent name must not be empty." },
					});
					continue;
				}
				if (!task.instruction) {
					resultsByTaskNumber.set(task.taskNumber, {
						task,
						outcome: { status: "invalid", reason: "empty-instruction", error: "Instruction must not be empty." },
					});
					continue;
				}
			...
			const resultTexts = results.map(({ outcome }) => {
