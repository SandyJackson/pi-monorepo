...
describe("subagent request schema", () => {
	it("accepts only the strict current request and task shape", () => {
		...
		expect(Value.Check(schema, { tasks: [{ agent: "alpha", instruction: "Review" }] , timeout: 1.5 })).toBe(false);
		expect(Value.Check(schema, {
...
const statusCases: Array<{
	...
	},
	{
		...
		outcome: { status: "launch_indeterminate", error: "socket closed", possiblePaneId: "possible-pane" },
		...
		expectedOutcomeDetails: { status: "launch_indeterminate", error: "socket closed", possiblePaneId: "possible-pane" },
	},
	{
...
describe("subagent result presentation", () => {
	it.each(statusCases)("presents $name with precise text and normalized details", async ({ outcome, timeout, text, expectedOutcomeDetails }) => {
	...
	it("numbers every precise status in a multiple-task result", async () => {
		const { tool, executeDelegation } = createTool();
		executeDelegation.mockImplementation(async (tasks) => tasks.map((task, index) => ({
			task,
	...
	it("returns a successful single task answer directly from its exact entry reference", async () => {
		await withSessionFile([
			{ id: "answer-1", type: "message", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "Exact delegated answer." }] } },
			{ id: "answer-2", type: "message", message: { role: "assistant", stopReason: "stop", content: [{ type: "text", text: "A later turn." }] } },
		], async (sessionPath) => {
			const { tool, executeDelegation } = createTool();
			executeDelegation.mockImplementation(async (tasks) => [{
				task: tasks[0],
	...
	it.each([
		{
			...
			excluded: "line-2000",
			notice: "[Answer truncated: showing 2000 of 2001 lines (18.4KB of 18.5KB). The full answer remains available in the referenced subagent session entry.]",
		},
		{
			name: "byte limit",
			...
			notice: "[Answer truncated: showing 25 of 30 lines (48.9KB of 58.7KB). The full answer remains available in the referenced subagent session entry.]",
		},
	])("uses Pi's canonical head truncation at the $name", async ({ answer, expectedLast, excluded, notice }) => {
		await withSessionFile([{
			id: "answer",
			type: "message",
...
describe("subagent task validation", () => {
	it("executes valid siblings with their original task numbers and merges outcomes by request position", async () => {
		...
		expect(text.indexOf("### Task 1 — (empty agent) — invalid")).toBeLessThan(text.indexOf("### Task 2 — alpha — completed"));
		expect(text.indexOf("### Task 2 — alpha — completed")).toBeLessThan(text.indexOf("### Task 3 — missing — invalid"));
		expect(text).toContain("The delegated turn completed without a final answer. Inspect the visible session in pane pane-2.");
		expect(result.details).toEqual({
			tasks: [
				{ taskNumber: 1, agent: "", status: "invalid", reason: "empty-agent", error: "Agent name must not be empty." },
				{
					taskNumber: 2,
					agent: "alpha",
					status: "completed",
	...
	it("returns positional invalid outcomes without initializing delegation when every task is invalid", async () => {
		...
		expect(result.details).toEqual({
			tasks: [
				{ taskNumber: 1, agent: "", status: "invalid", reason: "empty-agent", error: "Agent name must not be empty." },
				{ taskNumber: 2, agent: "alpha", status: "invalid", reason: "empty-instruction", error: "Instruction must not be empty." },
				{
					taskNumber: 3,
					agent: "missing",
					status: "invalid",
...
describe("subagent listing", () => {
	it.each([{}, { tasks: [] }])("lists the snapshotted catalog without initializing delegation", async (params) => {
		const { tool, executeDelegation } = createTool();
