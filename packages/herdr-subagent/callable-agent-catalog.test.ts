...
describe("agent discovery", () => {
	it("preserves parsing, nearest-project traversal, and project precedence", () => {
		...
		fs.writeFileSync(path.join(nearerAgentsDir, "missing-description.md"), "---\nname: ignored\n---\n");

		const merged = mergeAgentLists(
			discoverUserAgents(),
			discoverProjectAgents(nestedDir),
		);
		...
		expect(merged.find(({ name }) => name === "shared")?.description).toBe(
			"project definition",
		);
...
describe("callable agent catalog lifecycle", () => {
	it("keeps description, listing, and resolution on one snapshot until the next session", async () => {
		...
		writeAgent(projectAgentsDir, "project-stable.md", "project-stable", "initial project definition");
